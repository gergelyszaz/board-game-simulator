...
public class ActionManager {
	...
	public boolean Step() throws IllegalAccessException {
		...
		if(nextAction==null) {
			if(actionStack.size()>1)	stepOutNested();
			else reset();
			return true;
		}
	 ...
	 */
	public void stepIntoNested() throws IllegalAccessException {
	...
	private static Action getNextAction(List<Action> actions, Action action)
		...
		if(nextActionIndex==0) throw new IllegalAccessException(action +" is " +
			 "not part of "+actions);
		if(nextActionIndex==actions.size())
			return null;
		else return actions.get(nextActionIndex);
	}
