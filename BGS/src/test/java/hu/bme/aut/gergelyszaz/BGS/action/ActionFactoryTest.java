...
public class ActionFactoryTest {
	...
	hu.bme.aut.gergelyszaz.bGL.Action assingmentAction, destroyAction,
	...
	public void setUp() throws Exception {
		...
		destroyAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(destroyAction.getName()).thenReturn(ActionFactory.DESTROY);

		endTurnAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(endTurnAction.getName()).thenReturn(ActionFactory.ENDTURN);

		ifAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(ifAction.getName()).thenReturn(ActionFactory.IF);
		when(ifAction.getNestedAction()).thenReturn(nestedAction);

		loseAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(loseAction.getName()).thenReturn(ActionFactory.LOSE);

		moveAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(moveAction.getName()).thenReturn(ActionFactory.MOVE);

		rollAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
		when(rollAction.getName()).thenReturn(ActionFactory.ROLL);

		selectAction = mock(hu.bme.aut.gergelyszaz.bGL.Action.class);
